...
- `rowcount` is an additive read-only property sourced identically on both
  cursors, so both cursors agree by construction.
- On a **TLS** connection the sync edge is `NotEligible`, so the sync cursor
  transparently falls back to the `block_on` path (byte-identical rows).
...
def test_default_cursor_fetchone_plaintext(plaintext_server):
    conn = _connect(_ctx(plaintext_server))
    try:
        cur = conn.cursor()
        cur.execute("SELECT 1")
...
def test_sync_matches_default_rows_plaintext(plaintext_server):
    """Sync fetch is byte-identical to the default block_on path for the same statement."""
    conn = _connect(_ctx(plaintext_server))
    try:
...
def test_rowcount_sync_equals_default_select(plaintext_server):
    conn = _connect(_ctx(plaintext_server))
    try:
        ac = conn.cursor()
        ac.execute("SELECT 1")
...
def test_sync_then_default_reuse(plaintext_server):
...
def test_default_then_sync_reuse(plaintext_server):
    conn = _connect(_ctx(plaintext_server))
    try:
        ac = conn.cursor()
        ac.execute("SELECT 1")
...
# ---------------------------------------------------------------------------
# TLS connection — sync edge NotEligible, transparent block_on fallback
# ---------------------------------------------------------------------------


def test_sync_cursor_tls_fallback(tls_server):
    """On a TLS connection the sync edge is NotEligible; the sync cursor falls
    back to the `block_on` path and returns byte-identical rows."""
