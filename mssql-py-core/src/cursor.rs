...
impl PyCoreCursor {
    ...
    fn execute(
        ...
        // Control-plane execute stays async; it collapses forward to the first
        // row-returning result set before any flip to the reactor-free sync edge
        // (rule C).
        let on_rows = py.detach(|| pyclient::run_execute(&cell, &handle, query, 30))?;
