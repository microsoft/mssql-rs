...
- **A2 remains recommended** so `SQL_ATTR_RESET_CONNECTION` is safe regardless of the short-circuit — implemented as arm-time cache invalidation plus ack tracking rather than an eager round trip, letting the pool discard a failed-reset connection at checkout without paying an extra RTT.
...
- **D6 — "acked before checkout" is carried by the isolation SET.** The pool's checkout `SET TRANSACTION ISOLATION LEVEL READ COMMITTED` (D9/#343) is the request that carries the armed reset bit, so msodbcsql/mssql-python effectively process+ack the reset within that checkout call (when it emits I/O). **Refined by A2/B4:** when the borrower never changed isolation the checkout SET equals the cached value and would short-circuit with no I/O (`txn.rs:487`), so the driver suppresses that short circuit while a reset is armed, guaranteeing the SET carries the bit and can verify the acknowledgement — without the extra round trip an eager reset would cost.
...
### A2. Arm-time cache invalidation + ack tracking (see D6/D9/B4)
- mssql-python's pool issues `SET TRANSACTION ISOLATION LEVEL READ COMMITTED` right after `reset()` (D9), which normally carries the reset bit. **But** when the borrower never changed isolation, that SET equals the cached value and short-circuits with no I/O (`txn.rs:487`), deferring the reset to a later request. Acceptance criterion #4 ("reset processed and acknowledged before pool checkout succeeds") is met **without** an extra round trip: `prepare_reset_connection` drops the session-bound caches and login-default settings at arm time, and sets a `reset_pending` flag exposed as `TdsClient::reset_pending()`. The ODBC layer suppresses the isolation short-circuit while a reset is armed so the checkout SET always carries the bit, then checks `reset_pending()` after it to confirm the server acknowledged. An earlier revision drove a dedicated `SELECT 1` here; it was removed because the packet-header bit already guarantees reset-before-execute, so the RTT bought nothing that arm-time invalidation does not (see B4).
...
- **Chosen approach — arm, do not round-trip (no-RTT).** `SQL_ATTR_RESET_CONNECTION` arms the bit and returns without I/O. RESETCONNECTION is a TDS packet-header bit (MS-TDS 2.2.3.1.2), so the server returns the session to its login defaults *before* processing whichever request carries it — the next borrower can never see the previous borrower's state, regardless of when that request happens. A dedicated `SELECT 1` would roughly double checkout cost (measured ~1.65 ms vs ~0.80 ms on loopback, the delta being a full RTT, so it grows with network latency) and buys nothing for state correctness. The two properties that deferral would otherwise cost are kept without the round trip:
  - **No stale client-side state:** `prepare_reset_connection` drops the session-bound caches (prepared handles, AE describe metadata, capture state), the session-state table, and the negotiated DATABASE/LANGUAGE/COLLATION at *arm* time. Without this a borrower could be served a prepared handle that the carrying request's own header bit is about to drop server-side — exactly how msodbcsql produces native 8179 across a pool reset. msodbcsql already does this for db/language/collation (D4(d)); it simply does not extend it to prepared handles.
  - **Fail at checkout:** the DBC-level `pending_reset_ack` flag suppresses the same-value short circuit in `set_txn_isolation`, so mssql-python's checkout `SET TRANSACTION ISOLATION LEVEL READ COMMITTED` (D9/#343) always emits and carries the armed bit. That handler then verifies `TdsClient::reset_pending()` cleared — i.e. the server actually sent the `ResetConnection` ENVCHANGE — and on failure poisons the client and surfaces `08S01`, so the pool discards the connection before handing it to a borrower.
- **Degradation is graceful.** A consumer that never issues the checkout isolation SET gets msodbcsql's behavior exactly: the bit rides the borrower's first request, still reset-before-execute, only without the early discard. That is a loss of fail-fast, not of correctness.
- **Cost analysis that drove the no-RTT choice.** An eager `SELECT 1` inside the reset set-attr was implemented first and then removed. Measured on loopback (300 iterations, release), eager reset + first query was **1.63–1.71 ms** versus **0.79–0.83 ms** piggyback; the delta is a full request/response, so it scales with network RTT rather than staying at the loopback figure. Since the packet-header bit already guarantees reset-before-execute, that RTT bought only fail-fast and cache hygiene — both of which the arm-time cache clear plus the `pending_reset_ack` carrier now provide for free. See B4 for the mechanism.
