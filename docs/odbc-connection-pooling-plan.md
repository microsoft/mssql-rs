...
- **Cost of A2, the alternative considered, and why msodbcsql's piggyback is nonetheless correct.** The eager round trip roughly doubles checkout cost: measured on loopback (300 iterations, release), eager reset + first query is **1.63–1.71 ms** versus **0.79–0.83 ms** for the piggyback path, and the delta is a full request/response so it scales with network RTT rather than staying at the loopback figure.
  - **Piggyback does *not* leak server-side state, and msodbcsql is not buggy for shipping it.** RESETCONNECTION is a TDS *packet-header status bit* (`0x08`, MS-TDS 2.2.3.1.2) carried on the first packet of the next batch/RPC/TM request; the server resets the session to login defaults **before processing that request**. So even when the ack rides the borrower's first query, that query already executes against a clean session. Deferring the ack costs *when we learn about failure*, not *whether the next borrower sees dirty state*.
  - **What A2 actually buys.** (1) **Fail-at-checkout:** a reset that fails surfaces inside `Connection::reset()` so the pool discards the connection, instead of failing the borrower's first query. This is acceptance criterion #4, so it is a requirement, not a preference. (2) **Closing the client-side cache window:** our session-bound caches are cleared in `on_reset_connection_ack`, not at arm time, so until the ack lands `execute_prepared` (`tds_client.rs:2178`) can still resolve a *previous borrower's* prepared handle and compose an `sp_execute` against a handle the reset is about to drop. That is exactly the msodbcsql behavior our e2e pins: it keeps its handle cache across the reset and fails with native 8179, which `PreparedStatementSurvivesResetViaReprepare` documents as an intended divergence. (Note msodbcsql *does* eagerly restore its cached DATABASE/LANGUAGE/COLLATION at arm time — D4(d) — so it has no staleness gap for those; prepared handles are the one it does not clear.)
  - **Alternative (rejected, but viable):** invalidate `state.txn_isolation` as part of the reset so the pool's checkout `SQLSetConnectAttr(SQL_ATTR_TXN_ISOLATION, READ COMMITTED)` can no longer short-circuit and always emits the batch carrying the armed bit. Combined with clearing the session-bound caches at *arm* time (mirroring msodbcsql's D4(d) treatment of db/lang/collation) this would recover both benefits above with no extra RTT. It was **not** chosen because criterion #4's fail-at-checkout would then depend on the consumer issuing that isolation SET unconditionally on every checkout — true of mssql-python today, but a property of *their* call sequence rather than of our handler, and it would regress silently if a pool skipped or reordered it. The extra RTT is the accepted price for keeping the guarantee local. Revisit if checkout latency becomes a measured bottleneck.
