...
- **Cost of A2, and the alternative considered (deliberate accept).** The eager round trip roughly doubles checkout cost: measured on loopback (300 iterations, release), eager reset + first query is **1.63–1.71 ms** versus **0.79–0.83 ms** for the piggyback path, and the delta is a full request/response so it scales with network RTT rather than staying at the loopback figure. The alternative is to **invalidate the cached `state.txn_isolation` as part of the reset**, so the pool's next call — `SQLSetConnectAttr(SQL_ATTR_TXN_ISOLATION, READ COMMITTED)`, which mssql-python issues unconditionally on every checkout — can no longer short-circuit and always emits the batch carrying the armed bit, preserving fail-at-checkout with no extra round trip. **We deliberately keep A2**: it makes the guarantee a property of *our* handler rather than of the consumer's call order, so the reset still cannot be silently deferred if a pool skips the isolation SET, reorders it, or adopts a different checkout sequence. The extra RTT is the accepted price for not coupling correctness to another repo's call pattern. Revisit if checkout latency becomes a measured bottleneck.
