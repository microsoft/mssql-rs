...
use crate::{
    ...
    io::token_stream::{ParserContext, RowPauseState, RowReadResult},
...
impl TdsClient {
        ...
        loop {
            ...
            match result {
                ...
                RowReadResult::RowPaused(pause_state) => {
                    // Writer requested a pause after a column; resume the same row.
                    let start = Instant::now();
                    let result = self
                        .transport
                        .resume_row_into(
                    ...
                    match result {
                        ...
                        RowReadResult::RowPaused(next_pause) => {
                            // Re-enter the resume loop for writers that pause on every column.
                            return self.resume_row_loop(next_pause, writer).await;
                        }
    ...
    /// Loops calling `resume_row_into` until the row is fully written or a
    /// non-row token is received. Used when a `RowWriter` pauses on every column.
    async fn resume_row_loop(
        &mut self,
        pause_state: RowPauseState,
        writer: &mut (dyn RowWriter + Send),
    ) -> TdsResult<bool> {
        let mut current = pause_state;
        loop {
            let start = Instant::now();
            let result = self
                .transport
            ...
            match result {
                ...
                RowReadResult::Token(token) => {
                    ...
                    // If handle_row_read_token returns None, the loop consumed
                    // the token but there is no row yet — this should not happen
                    // mid-resume, but guard against it to avoid an infinite loop.
                    return Err(crate::error::Error::ProtocolError(
...
mod tests {
    ...
    use crate::io::token_stream::{ParserContext, RowPauseState, RowReadResult, TdsTokenStreamReader};
