...
mod test {
    ...
    mod vector_tests {
        ...
        fn test_vector_max_dimensions() {
            let max = VectorBaseType::Float32.max_dimensions();
            let dimensions: Vec<f32> = (0..max).map(|i| i as f32).collect();
            let vector = SqlVector::try_from_f32(dimensions).unwrap();
            assert_eq!(vector.dimension_count(), max);
        }
        ...
        fn test_vector_too_many_dimensions() {
            let dimensions: Vec<f32> = (0..(VectorBaseType::Float32.max_dimensions() + 1))
                .map(|i| i as f32)
                .collect();
