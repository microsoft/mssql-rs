// Copyright (c) Microsoft Corporation.
// Licensed under the MIT License.

//! Parser for the TABNAME TDS token (0xA4).
//!
//! TABNAME carries table name information for cursor result sets.
//! The payload is length-prefixed (u16) and can be safely skipped
//! because table name data is not required for result-set processing.

use async_trait::async_trait;

use super::common::TokenParser;
use crate::{core::TdsResult, io::packet_reader::TdsPacketReader};
use crate::{io::token_stream::ParserContext, token::tokens::Tokens};

/// Reads the TABNAME token's u16 length prefix and skips the body.
#[derive(Default)]
pub(crate) struct TabNameTokenParser;

#[async_trait]
impl<T> TokenParser<T> for TabNameTokenParser
where
    T: TdsPacketReader + Send + Sync,
{
    async fn parse(&self, reader: &mut T, _context: &ParserContext) -> TdsResult<Tokens> {
        let length = reader.read_uint16().await? as usize;
        reader.skip_bytes(length).await?;
        Ok(Tokens::TabName)
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::connection::transport::network_transport::tests::create_network_transport_with_data;
    use crate::io::packet_reader::tests::TestPacketBuilder;
    use crate::message::messages::PacketType;

    #[tokio::test]
    async fn test_tab_name_empty_body() {
        let mut builder = TestPacketBuilder::new(PacketType::PreLogin);
        builder.append_u16(0);
        let mut reader = create_network_transport_with_data(&builder.build()).await;

        let parser = TabNameTokenParser;
        let token = parser
            .parse(&mut reader, &ParserContext::default())
            .await
            .unwrap();
        assert!(matches!(token, Tokens::TabName));
    }

    #[tokio::test]
    async fn test_tab_name_with_body() {
        let mut builder = TestPacketBuilder::new(PacketType::PreLogin);
        builder.append_u16(4);
        builder.append_bytes(&[0xAA, 0xBB, 0xCC, 0xDD]);
        let mut reader = create_network_transport_with_data(&builder.build()).await;

        let parser = TabNameTokenParser;
        let token = parser
            .parse(&mut reader, &ParserContext::default())
            .await
            .unwrap();
        assert!(matches!(token, Tokens::TabName));
    }
}
