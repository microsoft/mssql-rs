...
mod client_based_iterators {
    ...
    /// ODBC-style writer: identical to [`SparseCaptureWriter`] but additionally
    /// pauses *before* the first column, mirroring how `SQLFetch` positions on a
    /// row without decoding any data and `SQLGetData` then drives column reads.
    #[derive(Default)]
    struct OdbcStyleWriter {
        inner: SparseCaptureWriter,
    }

    impl OdbcStyleWriter {
        fn new(col_count: usize) -> Self {
            Self {
                inner: SparseCaptureWriter::new(col_count),
            }
        }

        fn request_column(&mut self, requested_column: usize) {
            self.inner.request_column(requested_column);
        }

        fn clear_request(&mut self) {
            self.inner.clear_request();
        }
    }

    impl RowWriter for OdbcStyleWriter {
        fn pause_before_first_column(&self) -> bool {
            true
        }
        fn pause_after_column(&self, col: usize) -> bool {
            self.inner.pause_after_column(col)
        }
        fn write_null(&mut self, col: usize) {
    ...
    /// End-to-end ODBC positioning model: `SQLFetch` positions on a row while
    /// decoding no columns (pause-before-first-column), then each `SQLGetData`
    /// reads exactly one requested column, draining the columns in between. The
    /// last requested column is a PLP value streamed via `read_active_plp_bytes`.
    #[tokio::test]
    async fn odbc_pause_before_first_then_getdata_columns() -> mssql_tds::core::TdsResult<()> {
        let context = create_context();
        let provider = TdsConnectionProvider {};
        let mut client = provider
            .create_client(context, &build_tcp_datasource(), None)
        ...
        let mut row = OdbcStyleWriter::new(5);
        if let Some(rs) = client.get_current_resultset() {
            // SQLFetch: position on the row without reading any column. The writer
            // pauses before column 0, so nothing is captured yet.
            assert!(rs.next_row_into(&mut row).await?);
            assert!(
                row.inner.captured.is_empty(),
                "SQLFetch must not materialize any column"
            );

            // SQLGetData(2): read c2, draining c1 along the way.
            row.request_column(2);
            assert!(rs.next_row_into(&mut row).await?);
            assert_eq!(row.inner.captured.len(), 1);
            assert!(matches!(
                &row.inner.captured[0],
                ColumnValues::String(s) if s.to_utf8_string() == "row1-c2"
            ));

            // SQLGetData(4): PLP column, draining c3 first, then stream the value.
            row.request_column(4);
            assert!(rs.next_row_into(&mut row).await?);
            let mut buf = [0u8; 2048];
            let mut c4 = Vec::new();
            loop {
                let n = rs.read_active_plp_bytes(&mut buf).await?;
                c4.extend_from_slice(&buf[..n]);
                if rs.active_plp_reached_end() {
                    break;
                }
                assert!(n > 0, "Expected progress while draining c4");
            }
            assert_eq!(c4.len(), 9000);
            assert!(c4.iter().all(|b| *b == b'A'));

            // Drain the remainder of the row (c5) and confirm end-of-rowset.
            row.clear_request();
            assert!(rs.next_row_into(&mut row).await?);
            assert!(!rs.next_row_into(&mut row).await?);
        }
