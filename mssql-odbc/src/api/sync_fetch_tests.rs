...
/// (a) Differential: a full sync `SQLFetch` drain reproduces the async oracle
/// byte-identically, and the DBC actually flipped to the sync edge at execute time.
#[test]
fn sync_fetch_matches_async_oracle() {
...
/// (b) The sync fetch serves every good row then surfaces the mid-stream error on
/// the `SQLFetch` that hits the failing row — rows and error in the correct order.
#[test]
fn sync_fetch_surfaces_midstream_error_after_good_rows() {
    const QUERY: &str = "SELECT SYNC ERR ROWS";
    // 50 rows, error after 37 — each SQLFetch reads one row, so the error surfaces
    // on the fetch that hits row 38, after the 37 good rows have been served.
