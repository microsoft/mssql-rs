...
static SLOT_CURSOR: AtomicUsize = AtomicUsize::new(0);
...
enum Op {
    ...
    ReserveOnly,
    /// Floor: slot index + generation compare instead of the global map.
    LookupSlot,
    /// Floor: slot lookup plus the same ancestor reservation.
    AcquireSlot,
}
...
impl Op {
    fn label(self) -> &'static str {
        match self {
            ...
            Self::LookupSlot => "  lookupslot (index+generation)",
            Self::AcquireSlot => "acquireslot (slot + reservation)",
        }
    }
}
        ...
        let workers: Vec<_> = (0..threads)
            .map(|_| {
                ...
                scope.spawn(move || {
                    ...
                    let slot = SLOT_CURSOR.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                    let packed =
                        crate::handles::probe_slot_install(slot, 7, Arc::clone(&stmt));
                    barrier.wait();
                    for _ in 0..iters {
                        match op {
                            Op::BaseSync => {
                            ...
                            Op::LookupSlot => {
                                let v = crate::handles::probe_slot_lookup::<StmtHandle>(packed)
                                    .unwrap();
                                black_box(&*v);
                            }
                            Op::AcquireSlot => {
                                let v = crate::handles::probe_slot_lookup::<StmtHandle>(packed)
                                    .unwrap();
                                crate::handles::probe_reserve(v.activity());
                                black_box(&*v);
                            }
                            Op::GetDataSync => {
                                let h = handle_from_raw::<StmtHandle>(stmt_raw).unwrap();
