...
// reports the exact number of bytes still available before each copy and
// decrements as the value drains.
...
// For a value larger than one wire pump (> 8 KiB), the length is not yet known
// while the wire is still being drained, so the indicator is SQL_NO_TOTAL.
