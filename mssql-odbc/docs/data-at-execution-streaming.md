...
The in-flight request is lost, but the session is not. `abort_streamed_write`
clears the open-batch flag before dropping the transport, and the ODBC layer
returns the client to the connection rather than discarding it, so the next
execute reaches `check_and_reconnect`, observes the dead socket, and replays the
session state onto a fresh login. Idle Connection Resiliency is on by default
(`connect_retry_count = 1`), so this needs no connection-string opt-in. This is
the same lazy recovery msodbcsql performs in `GetBatchCtxOrRecover`
(`sqlctokn.cpp:4088`), which likewise runs at the top of the next API call
rather than at failure time. Recovery is declined while a transaction is open
(`is_recovery_possible`), so a DAE sequence that dies inside an explicit
transaction does end the connection.
...
- A DAE sequence that fails on the wire (rather than being cancelled or
  rejected) loses the request and the socket, because a request interrupted by a
  send or receive failure cannot be retracted with `EOM | IGNORE`. The session
  itself survives: the next execute reconnects transparently, as described under
  Error Recovery. Recovery is declined while a transaction is open.
