...
| P5 — Testing & end-to-end | 46582 | Implemented — the fetch type map is covered end to end on both paths for the date/time, integer, bit, float, GUID, decimal, money and character families, against msodbcsql ([#376](https://github.com/microsoft/mssql-rs/pull/376)). That coverage found two divergences, both fixed there: decimal and money rendered a leading zero below magnitude one where msodbcsql does not. Review of it surfaced a third — `SQLGetData` returned success for a NULL column read with no indicator, leaving the caller's buffer holding the previous row's value — which [#382](https://github.com/microsoft/mssql-rs/pull/382) fixed independently and first (AB#47507); msodbcsql answers 22002, as our own bound path already did. The `mssql-python` suite runs against a locally-swapped build on Linux and reproduces CI exactly (see above for current figures); the Windows equivalent is blocked on a pooling hang (Task 47510). Remaining gaps are tracked rather than open here: bound `SQL_C_BINARY` (Task 47239) and bound PLP/XML (Task 47361), both blocked on unimplemented delivery rather than on missing tests; and the fetch-path error semantics (22003 / 01S07 / 07006), which are unit-tested but have almost no parity coverage (Task 47678). |
