...
- **Binary targets: restored.** `SQL_C_BINARY` was implemented alongside the chunked-offset streaming, so removing that streaming (ceding it to [#153](https://github.com/microsoft/mssql-rs/pull/153)) took `SQL_C_BINARY` with it. It was re-expressed on the wire-stream model and restored in AB#47239. Binary → character (hex rendering) was never implemented in any phase and was added in AB#47240, on both the `SQLGetData` and bound-fetch paths. The e2e test that was anchored on `VARBINARY(8)` being unconvertible now asserts the target-side refusal instead (`GetDataLiveTest.UnsupportedTargetHyc00PreservesValue`, using `SQL_C_NUMERIC`).
...
| `YYYY/MM/DD` and the ODBC escape literals `{d '...'}` / `{t '...'}` / `{ts '...'}` | accepted (`rgbECODE_DATE_SLASH` retry, and the `FindECode` branch) | accepted | Parity - Task [47246](https://sqlclientdrivers.visualstudio.com/mssql-rs/_workitems/edit/47246). `OdbcTemporalLiteralsFromCharacterColumns` measures `SQLGetData` and `SQLBindCol`, including max columns, against msodbcsql18 `18.6.1.1-1` (`SQL_DRIVER_VER=18.06.0001`). See [the shared-parser grammar and evidence](parameters_plan.md#character-c-type-to-a-temporal-parametertype-ab47851). |
| `T` separator, `HH:MM` without seconds, unpadded fields such as `2023-6-5` | rejected (fixed-length token grammar) | accepted in plain text | Deliberately preserved; AB#47246 does not narrow existing accepted forms. |
