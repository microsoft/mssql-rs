...
| Character or decimal source into `SQL_C_TINYINT` above 127 | `22003` | `22003` | **Resolved.** `SQL_C_TINYINT` is the ODBC 2.x spelling of the signed form, so the signed limit applies; it was briefly read as `u8` here. Only `SQL_C_UTINYINT` is unsigned. |
